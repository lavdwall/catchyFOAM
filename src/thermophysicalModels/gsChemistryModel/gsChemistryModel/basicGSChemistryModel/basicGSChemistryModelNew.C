...
Foam::autoPtr<Foam::basicGSChemistryModel>
Foam::basicGSChemistryModel::New(rhoReactionThermo& thermo)
{
    ...
    const dictionary& solidThermoTypeDict(thermoDict.subDict("solidThermoType"));
    ...
    (
        word(chemistryTypeDict.lookup("solver")) + '<'
      + word(chemistryTypeDict.lookup("method")) + '<'
      + typeName + ','
      + gasThermoTypeName + ',' + solidThermoTypeName + ">>"
    );
    ...
    return
        autoPtr<basicGSChemistryModel>(cstrIter()(thermo));
}
