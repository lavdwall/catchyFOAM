...
void Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::
initializeSurface()
{
...
template<class ReactionThermo, class GThermoType, class SThermoType>
Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::PhaseGSChemistryModel
(
...
:
    ...
    (
        IOobject
        (
            thermo.phasePropertyName("thermo:rhog"),
        ...
        this->mesh(),
        dimensionedScalar("one", dimDensity, 0.0),
    ...
    (
        IOobject
        (
            thermo.phasePropertyName("alpha_chem"),
            this->mesh().time().timeName(),
            this->mesh(),
            IOobject::NO_READ,
            IOobject::NO_WRITE
    ...
    alphaMin_(this->lookupOrDefault("alphaMin",0.0)),
    twoPhaseAlpha_(this->lookupOrDefault("twoPhaseAlpha",false))
{
    ...
    if (twoPhaseAlpha_)
    {
        Info << "Using coupled chemistry approach for twoPhaseSystem" << endl;
    }
    ...
    forAll(rhog_, celli)
    {
        ...
        (
            this->thermo()
        ).gasCellMixture(celli).rho(this->thermo().p()[celli], this->thermo().T()[celli]);
        rhog_[celli] = rhoi;
    }
}
...
template<class ReactionThermo, class GThermoType, class SThermoType>
Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::
...
template<class ReactionThermo, class GThermoType, class SThermoType>
void Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::omega
(
    const scalar p,
    const scalar T,
    const scalarField& c,
...
template<class ReactionThermo, class GThermoType, class SThermoType>
Foam::scalar Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::omegaGI
(
    const label index,
    const scalar p,
    const scalar T,
...
template<class ReactionThermo, class GThermoType, class SThermoType>
Foam::scalar Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::omegaSI
(
    const label index,
    const scalar p,
    const scalar T,
...
template<class ReactionThermo, class GThermoType, class SThermoType>
void Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::derivatives
(
    const scalar time,
    const scalarField& c,
    const label li,
...
template<class ReactionThermo, class GThermoType, class SThermoType>
void Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::jacobian
(
    const scalar t,
    const scalarField& c,
    const label li,
...
Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::tc() const
{
    NotImplemented;
...
template<class ReactionThermo, class GThermoType, class SThermoType>
Foam::tmp<Foam::volScalarField>
Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::Qdot() const
{
...
Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::calculateRR
(
    const label ri,
    const label si
) const
...
template<class ReactionThermo, class GThermoType, class SThermoType>
void Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::calculate()
{
    if (!this->chemistry_)
    {
        return;
    ...
    forAll(T, celli)
    {
        ...
        if (alpha_[celli] < alphaMin_ && !twoPhaseAlpha_) continue;
        if (twoPhaseAlpha_)
        {
            por_ = (fpor_ - 1.0)*alpha_[celli] + 1.0;
            multiplyAV_ = alpha_[celli];
        }
        else
        {
            multiplyPor_ = alpha_[celli];
            multiplyAV_ = alpha_[celli];
        }
...
Foam::scalar Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::solve
(
...
)
{
    forAll(Tg, celli)
    {
        this->thermo().T()[celli] = Tg[celli];
    }
    this->thermo().he() = this->thermo().he(this->thermo().p(), this->thermo().T());
}


template<class ReactionThermo, class GThermoType, class SThermoType>
Foam::scalar Foam::PhaseGSChemistryModel<ReactionThermo, GThermoType, SThermoType>::
getRatesQdotI
(
    const scalar deltaT,
    const label celli,
