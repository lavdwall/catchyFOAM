...
)
{
    ...
    if (t.isWord())
    {
        ...
        else if (sspecies.found(specieName))
        {
            index = sspecies[specieName];
            isgas = false;
        }
        else
        {
            FatalIOErrorInFunction(is)
                ...
                << " and not found in table " << sspecies
                << exit(FatalIOError);
...
)
{
    ...
    while (is.good())
    {
        dlrhs.append(specieCoeffs(species, species, is));
...
void Foam::specieCoeffs::setLRhsGS
(
    Istream& is,
    const speciesTable& species,
    const speciesTable& sspecies,
...
)
{
    ...
    while (is.good())
    {
        dlrhs.append(specieCoeffs(species, sspecies, is));
