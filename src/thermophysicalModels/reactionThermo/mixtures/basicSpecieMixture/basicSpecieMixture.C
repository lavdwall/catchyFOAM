...
:
    basicMixture(thermoDict, mesh, phaseName),
    species_(gasSpecieNames),
    solidSpecies_(solidSpecieNames),
    active_(species_.size()+solidSpecies_.size(), true),
    Y_(species_.size()),
    Ys_(solidSpecies_.size())
{
    tmp<volScalarField> tYdefault;
    ...
    tmp<volScalarField> tYsdefault;

    forAll(solidSpecies_, i)
    {
        IOobject header
        (
            IOobject::groupName(solidSpecies_[i], phaseName),
            mesh.time().timeName(),
            mesh,
            IOobject::NO_READ
        );
        ...
        else
        {
            ...
            if (!tYsdefault.valid())
            {
                word YsdefaultName(IOobject::groupName("Ysdefault", phaseName));
                ...
                if (timeIO.typeHeaderOk<volScalarField>(true))
                {
                    tYsdefault = new volScalarField(timeIO, mesh);
                }
                else if (constantIO.typeHeaderOk<volScalarField>(true))
                {
                    tYsdefault = new volScalarField(constantIO, mesh);
                }
                else
                {
                    tYsdefault = new volScalarField(time0IO, mesh);
                }
            }
