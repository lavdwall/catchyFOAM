...
T0 = 1423.0  # Temperature 
P0 = 1.1e5   # Pressure
Y0 = {'CH4':0.66667, 'O2':0.33333} # Composition

epsB = 0.45            # Voidage
epsC = 0.0             # Catalyst porosity
velocity = 2.5         # Interstitial inlet velocity
cat_area_per_vol = 0.0 # Catalyst surface area per unit reactor(!) volume
...
pfr = surfacePFR(gas, surf, 1.0, ID, epsB, cat_area_per_vol, epsC = epsC, isothermal=True)
...
ax2.plot(ofdataG['z'], ofdataG['O2gas'], '--', linewidth=2, color='crimson', label='catchyFOAM')
...
ax3.plot(ofdataG['z'], ofdataG['N2gas'], '--', linewidth=2, color='crimson', label='catchyFOAM')
...
ax6.plot(ofdataG['z'], ofdataG['Tgas'], '--', linewidth=2, color='crimson', label='catchyFOAM')
